...
video_directory = "/mnt/4tb/data/bobslmouth_mask"
...
split_index = int(0.95 * len(video_files))
...
for video_file in train_videos:
    video_id = video_file.replace(".mp4", "")  # e.g., 'n_001_002_000_nadir'
    word = video_id.split('_')[-1]             # Extract the word, e.g., 'nadir'
    word = word.replace("-", " ")
    annotation[video_id] = {
        "folder": "/mnt/4tb/data/bobslmouth_mask",
        "mplug": word,
        "polish_mplug": "",
        "ofa": "",
        "sound_mplug": "",
...
output_file = "data/bobsl_mouthed/annotation.json"
...
notes_file = "data/bobsl_mouthed/notes.txt"
